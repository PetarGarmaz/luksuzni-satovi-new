...
const WatchDetailClient = ({ watch, brandSlug }) => {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="pt-32 pb-16 px-6">
          <div className="max-w-4xl mx-auto text-center">
            <Link href={brandSlug ? `/prodaja-satova/${brandSlug}` : '/prodaja-satova'}>
              <Button
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="pt-20 sm:pt-32 pb-8 px-4 sm:px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center mb-6 text-sm flex-wrap gap-y-1">
            ...
            <span className="mx-2 text-gray-400">/</span>
            <Link href={`/prodaja-satova/${brandSlug}`} className="text-gray-500 hover:text-gray-700">{watch.brand}</Link>
            <span className="mx-2 text-gray-400">/</span>
            <span className="text-gray-900">{watch.name}</span>
          </div>
          <Link href={`/prodaja-satova/${brandSlug}`} className="inline-flex items-center text-gray-600 hover:text-gray-900 transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-16">
            ...
            <div className="space-y-6">
              <div>
                ...
                <h1 className="text-3xl sm:text-4xl font-light text-gray-900 mb-2">{watch.brand}</h1>
                <h2 className="text-xl sm:text-2xl text-gray-700 mb-4">{watch.name}</h2>
                <p className="text-3xl sm:text-4xl font-light text-gray-900 mb-6">{formatPrice(watch.price)}</p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    {watch.hasBox ? <Check className="w-4 h-4 text-green-600" /> : <X className="w-4 h-4 text-red-500" />}
                    <span className={watch.hasBox ? 'text-gray-900' : 'text-gray-500'}>{t.watchDetail.originalBox}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {watch.hasPapers ? <Check className="w-4 h-4 text-green-600" /> : <X className="w-4 h-4 text-red-500" />}
                    <span className={watch.hasPapers ? 'text-gray-900' : 'text-gray-500'}>{t.watchDetail.documentation}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {watch.hasWarranty ? <Check className="w-4 h-4 text-green-600" /> : <X className="w-4 h-4 text-red-500" />}
                    <span className={watch.hasWarranty ? 'text-gray-900' : 'text-gray-500'}>{t.watchDetail.warranty}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                  ...
                  <div className="flex flex-col items-center">
                    <Award className="w-8 h-8 mb-2" style={{ color: '#bd890f' }} />
